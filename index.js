...
const productsBox = document.querySelector('.shopping-content');

//Adding products to shopping content
...
productsData.forEach(({ id, img, name, text, price }) => {
    ...
    <div class="product-add"><svg width="27" height="25" viewBox="0 0 27 25" fill="none"
...
// Elements for cart scripts

const addButtons = document.querySelectorAll('.product-add');

const cartBox = document.querySelector('.cart');

let cartCount = document.querySelectorAll('.cartProduct');
...
const cartSizeIndicator = document.querySelector('.cart_size_indicator');
    ...
    button.addEventListener('click', () => {
        ...
        productsData.forEach(({ id, img, name, price, size, color }) => {
            ...
            `;
                ...
                cartCount = document.querySelectorAll('.cartProduct');

                cartSizeIndicator.style.display = "flex";
                cartSizeIndicator.textContent = cartCount.length;

                //Cart items deleting

                const delButtons = document.querySelectorAll('.btn_del');

                delButtons.forEach(delbutton => {
                    delbutton.addEventListener('click', () => {
                        const delprod = delbutton.closest('.cartProduct');
                        delprod.remove();
                        cartCount = document.querySelectorAll('.cartProduct');
                        cartSizeIndicator.textContent = cartCount.length;
